...
import {
	applyRenames,
	renamesFromMigrations,
} from "../dist/migrations/codemod.js";
...
test("renames all BT input aliases", () => {
	...
	for (const [input, expected] of pairs) {
		assert.equal(
			applyRenames(input, renames).text,
			expected,
			`${input} should become ${expected}`,
		);
	}
});
...
test("renames configure() object keys but not the already-new name", () => {
	...
	const after =
		"return { isOverlayEnabled: true, isDetectingDroppedFrames: false };";
	assert.equal(applyRenames(before, renames).text, after);

	const already = "return { isOverlayEnabled: true };";
	assert.equal(
		applyRenames(already, renames).changed,
		false,
		"the new key name must be left alone",
	);
});
...
test("flags generic bootstrap option keys for review instead of renaming them", () => {
	...
	const input =
		'bootstrap(Game, { canvasId: "c", containerId: "wrap", waitForDOMReady: true });';
	const result = applyRenames(input, renames);

	assert.equal(
		result.changed,
		false,
		"generic bootstrap keys must not be auto-renamed",
	);
...
test("auto-renames distinctive method names", () => {
	assert.equal(
		applyRenames("sheet.isIndexized()", renames).text,
		"sheet.isIndexed()",
	);
	assert.equal(
		applyRenames("rect.containsXY(x, y)", renames).text,
		"rect.isContainingXY(x, y)",
	);
	assert.equal(
		applyRenames("a.intersectionTo(b, out)", renames).text,
		"a.intersectTo(b, out)",
	);
});
...
test("flags common-word methods for review instead of rewriting them", () => {
	const result = applyRenames(
		"if (a.equals(b) && r.contains(p) && r.intersects(o)) {}",
		renames,
	);
	...
	assert.equal(
		result.changed,
		false,
		"review-only renames must not change the text",
	);
	...
	assert.ok(
		equalsHit.after.includes("isEqual"),
		"review hit should suggest the new name",
	);
});
...
test("containsXY does not collide with the contains review rename", () => {
	...
	assert.equal(
		...
		0,
		"containsXY must not also trigger the contains review match",
	);
});
...
test("is idempotent: a second pass changes nothing", () => {
	const once = applyRenames(
		"BT.buttonDown(BT.BTN_A); sheet.isIndexized();",
		renames,
	);
	...
	assert.equal(
		twice.changed,
		false,
		"already-migrated code should not change again",
	);
});

test("migrationsThrough includes the 1.x rename migration and excludes future ones", () => {
	assert.equal(
		migrationsThrough("1.0.0").length,
		1,
		"the 2026-05-31 api-naming migration applies from 1.0.0",
	);
	assert.equal(
	...
	assert.equal(
		...
		0,
		"does not apply below its since version",
	);
});

test("applyRenames rewrites importPath specifiers without a leading dot", () => {
	const importPathRename = [
		{ from: "blit386", to: "blit386", kind: "importPath", safety: "auto" },
	];
	...
	assert.equal(
		applyRenames(esm, importPathRename).text,
		"import { BT } from 'blit386';",
		"ESM import rewrites",
	);
	assert.equal(
		applyRenames(cjs, importPathRename).text,
		"const bt = require('blit386');",
		"CJS require rewrites",
	);
	assert.equal(
		applyRenames(scoped, importPathRename).text,
		scoped,
		"longer package name is not touched",
	);
});
...
test("every rename declares the fields the engine needs", () => {
	for (const migration of MIGRATIONS) {
		for (const rename of migration.renames) {
			...
			assert.ok(
				["memberCall", "objectKey", "method", "importPath"].includes(
					rename.kind,
				),
				"kind must be known",
			);
			assert.ok(
				["auto", "review"].includes(rename.safety),
