...
- `blit` CLI, adapter (`packages/kit/src/adapters.ts`), or `blit agents sync` changes: update both package READMEs and
  the design doc.
- `BLIT386_RANGE` or preflight/script changes: update skills, `CLAUDE.md`, and cross-repo notes when relevant.
- blit386 public API changes (sibling repo): audit kit docs and `packages/kit/content/rules/blit-api-names.md`.
